...
def count_schematics(gf180mcu_cells_dir, gf180mcu_schematics_dir):
    spice_cells = set()
    for root, _, files in os.walk(gf180mcu_cells_dir):
        for f in files:
            if f.endswith(".cdl"):
                full_path = os.path.join(root, f)
                rel_path = os.path.relpath(full_path, gf180mcu_cells_dir)
                key = rel_path[:-len(".cdl")]
                spice_cells.add(key)

    schematic_cells = set()
    for root, _, files in os.walk(gf180mcu_schematics_dir):
        for f in files:
            if f.endswith(".sch"):
                full_path = os.path.join(root, f)
                rel_path = os.path.relpath(full_path, gf180mcu_schematics_dir)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(f"Usage: python3 {sys.argv[0]} <gf180mcu_cells_dir> <gf180mcu_schematics_dir> <progress_svg>")
        sys.exit(1)

    gf180mcu_cells_dir = sys.argv[1]
    gf180mcu_schematics_dir = sys.argv[2]
    progress_svg = sys.argv[3]

    num_finish, num_total = count_schematics(gf180mcu_cells_dir, gf180mcu_schematics_dir)
